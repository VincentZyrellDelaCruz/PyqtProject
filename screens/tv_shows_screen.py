...
from controllers.movie_api_client import fetch_popular_tv_shows_sync, get_image_url
...
class TVShowsScreen(DetailViewMixin, QWidget):
    ...
    def init_ui(self):
        ...
        tv_section = self.create_tv_shows_section()
        self.main_layout.addWidget(tv_section)

    def create_tv_shows_section(self):
        container = QWidget()
        ...
        for show in shows:
            show_card = self.create_tv_card(show)
            grid_layout.addWidget(show_card, row, col)
            col += 1
            if col >= max_cols:
                col = 0
                row += 1
